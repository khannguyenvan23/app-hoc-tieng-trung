...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  function navLinkClass(active: boolean, mobile = false) {
    ...
    return active
      ...
      : `${base} text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 hover:text-zinc-950`;
  }
    ...
    <div className="app-bg min-h-screen overflow-x-clip text-zinc-950 dark:text-zinc-50">
      ...
      <header className="sticky top-0 z-40 border-b border-zinc-200/80 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:py-4">
          ...
          <nav className="hidden items-center gap-1 rounded-xl border border-zinc-200 dark:border-white/10 bg-white/80 p-1 text-sm shadow-sm sm:flex">
            {navItems.map((item) => {
          ...
          <details className="relative sm:hidden">
            <summary className="cursor-pointer list-none rounded-lg border border-zinc-300 dark:border-white/15 bg-white dark:bg-white/5 px-3 py-2 text-sm font-semibold shadow-sm hover:bg-zinc-100">
              Menu
            </summary>
            <nav className="absolute right-0 z-50 mt-2 grid w-52 gap-1 rounded-xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/5 p-2 text-sm shadow-xl">
              {navItems.map((item) => {
                const active = isActivePath(item.paths);
