      ...
      <header className="border-b border-zinc-200 bg-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-5 py-4">
          ...
          <nav className="flex flex-wrap items-center justify-end gap-2 text-sm">
            <Link
              className="rounded-md px-3 py-2 text-zinc-700 hover:bg-stone-100 hover:text-teal-800"
              href="/hsk1"
            >
              HSK1
            </Link>
            ...
            <Link
              className="rounded-md border border-zinc-300 px-3 py-2 font-medium hover:bg-stone-100"
              href="/login"
            >
              Đăng nhập
            </Link>
      ...
      <section className="border-b border-zinc-200 bg-white">
        <div className="mx-auto max-w-5xl px-5 py-14 sm:py-16">
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            ...
            <Link
              className="rounded-md border border-zinc-300 px-5 py-3 text-sm font-semibold hover:bg-stone-100"
              href="/pricing"
            >
              Xem bảng giá
            </Link>
          </div>
          <div className="mt-8 flex flex-wrap gap-2 text-sm text-zinc-700">
            {page.highlights.slice(0, 4).map((highlight) => (
              <span
                className="rounded-md border border-teal-100 bg-teal-50 px-3 py-2 text-teal-900"
                key={highlight}
              >
                {highlight}
              </span>
        ...
        <div className="mx-auto grid max-w-5xl gap-8 px-5 md:grid-cols-[0.8fr_1.2fr]">
          ...
          <ol className="space-y-3">
            {page.steps.map((step, index) => (
              <li
                className="flex gap-4 rounded-lg border border-zinc-200 bg-stone-50 p-4"
                key={step}
              >
        ...
        <div className="mx-auto max-w-5xl px-5">
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            {page.related.map((link) => (
              <Link
                className="rounded-md border border-zinc-300 px-4 py-2 text-sm font-medium hover:bg-stone-100 hover:text-teal-800"
                href={link.href}
                key={link.href}
              >
                {link.label}
