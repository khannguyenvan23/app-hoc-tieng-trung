    ...
    <>
      <section className="border-b border-zinc-200 dark:border-white/10 bg-white dark:bg-[#171a19]">
        <div className="mx-auto grid max-w-6xl gap-8 px-5 py-14 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              {phoneDigits ? (
                ...
                <span className="inline-flex min-h-11 items-center justify-center rounded-md bg-zinc-200 dark:bg-white/15 px-5 py-3 text-sm font-semibold text-zinc-700 dark:text-zinc-300">
                  Chưa cấu hình số điện thoại
                </span>
              )}
              {zaloDigits ? (
                <a
                  className="inline-flex min-h-11 items-center justify-center rounded-md border border-zinc-300 dark:border-white/15 bg-white dark:bg-[#171a19] px-5 py-3 text-sm font-semibold hover:bg-zinc-50 dark:hover:bg-white/5"
          ...
          <div className="rounded-lg border border-zinc-200 dark:border-white/10 bg-stone-50 dark:bg-white/5 p-5">
            {signedIn ? (
              <div className="mb-4 rounded-md border border-teal-100 dark:border-teal-500/30 bg-teal-50 dark:bg-teal-500/15 p-4">
                <div className="flex flex-wrap items-end justify-between gap-3">
                  <div>
                    <div className="text-xs font-medium uppercase text-teal-800 dark:text-teal-300">
                      Credit còn lại
                    </div>
                ...
                {creditError ? (
                  <p className="mt-3 text-sm text-red-700 dark:text-red-300">{creditError}</p>
                ) : null}
              </div>
            ) : null}
            <h2 className="text-lg font-semibold">Cách dùng credit</h2>
            <div className="mt-4 divide-y divide-zinc-200 rounded-md border border-zinc-200 dark:border-white/10 bg-white dark:bg-[#171a19]">
      ...
      <section className="mx-auto max-w-6xl px-5 py-12">
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {creditPlans.map((plan) => (
            ...
            >
              {plan.highlight ? (
                <div className="mb-3 inline-flex rounded-full bg-teal-50 dark:bg-teal-500/15 px-3 py-1 text-xs font-medium text-teal-800 dark:text-teal-300">
                  Gợi ý
                </div>
              ) : null}
              <h2 className="text-lg font-semibold">{plan.name}</h2>
              <div className="mt-4 text-3xl font-semibold">{plan.price}</div>
              <div className="mt-2 text-sm font-medium text-teal-800 dark:text-teal-300">
                {plan.credits}
              </div>
              <p className="mt-3 min-h-20 text-sm leading-6 text-zinc-600 dark:text-zinc-400">
                {plan.description}
              </p>
              {phoneDigits ? (
                <a
                ...
                <div className="mt-5 rounded-md border border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-white/5 px-3 py-2 text-sm text-zinc-600 dark:text-zinc-400">
      ...
      <section className="border-y border-zinc-200 dark:border-white/10 bg-white dark:bg-[#171a19]">
      ...
      <footer className="bg-white dark:bg-[#171a19]">
      ...
      <header className="border-b border-zinc-200 dark:border-white/10 bg-white dark:bg-[#171a19]">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-5">
          ...
          <nav className="flex items-center gap-2 text-sm">
            <Link
              className="rounded-md px-3 py-2 hover:bg-zinc-100 dark:hover:bg-white/10"
              href="/login"
            >
              Đăng nhập
            </Link>
